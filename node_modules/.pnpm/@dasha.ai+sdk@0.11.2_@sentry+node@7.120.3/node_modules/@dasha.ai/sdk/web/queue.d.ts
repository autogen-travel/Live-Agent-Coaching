...
import { Conversation, ConversationResult, DynamicToolDescription, DynamicToolScheme } from "./conversation";
...
export declare class ConversationQueue<TInput extends Record<string, unknown>, TOutput extends Record<string, unknown>> extends EventEmitter<{
    ...
    timeout: [key: string, jobId: string];
    rejected: [key: string, error: errors.ConversationRejectedError, jobId: string];
}> {
    private _log;
...
export declare class QueuedConversation<TInput extends Record<string, unknown>, TOutput extends Record<string, unknown>> extends EventEmitter<{
    ...
    private _recordingUrls?;
    private _additionalInformation?;
    setDynamicToolRaw(options: DynamicToolDescription, handler: (args: Record<string, unknown>, conv: Conversation<TInput, TOutput>) => unknown): Promise<void>;
    setDynamicTool<T extends TSchema>(options: DynamicToolScheme<T>, handler: (args: Partial<Static<T>>, conv: Conversation<TInput, TOutput>) => unknown): Promise<void>;
    /**
