...
function validateAccountConfig(log, value) {
    ...
    if (!json_1.AccountConfig.validate(value)) {
        (0, log_1.logAndThrow)(log, new errors.InternalError((_a = json_1.AccountConfig.validate.errors) === null || _a === void 0 ? void 0 : _a.join("\n")));
    }
}
function parseJson(log, content) {
    try {
        ...
        (0, log_1.logAndThrow)(log, new errors.InternalError(error.message, error));
    }
}
async function writeAccountConfig(log, config) {
    ...
    if (envPaths) {
        ...
        const configPath = path_1.default.join(configDir, "accounts.json");
        try {
            await fs_1.default.promises.mkdir(configDir, { recursive: true });
            await fs_1.default.promises.writeFile(configPath, configContent);
        }
        catch (error) {
...
async function readAccountConfig(log) {
    ...
    if (envPaths) {
        ...
        let configContent;
        try {
            configContent = await fs_1.default.promises.readFile(configPath, {
                encoding: "utf8",
            });
        }
        catch (error) {
            ...
            (0, log_1.logAndThrow)(log, new errors.FileSystemError(error));
        }
...
async function getAccount(name) {
    ...
    if (account === undefined) {
        (0, log_1.logAndThrow)(log, new errors.AccountNotFoundError(name));
    }
    return account;
}
/**
...
async function renameAccount(oldName, newName, options) {
    ...
    if (!(options === null || options === void 0 ? void 0 : options.overwrite) && newName in config.accounts) {
        (0, log_1.logAndThrow)(log, new errors.AccountExistsError(newName));
    }
...
async function _getEffectiveAccount(options) {
    ...
    const customerId = process === null || process === void 0 ? void 0 : process.env["DASHA_CUSTOMER_ID"];
    ...
    if (process !== undefined) {
        ...
        if (apiKey !== undefined) {
            ...
            return { server, apiKey, insecure, customerId };
        }
    }
    const config = await readAccountConfig(log);
    const currentAccountName = config.currentAccount;
    if (currentAccountName === undefined) {
        (0, log_1.logAndThrow)(log, new errors.Error("no account is set as current; did you forget to `dasha account login`?"));
    }
