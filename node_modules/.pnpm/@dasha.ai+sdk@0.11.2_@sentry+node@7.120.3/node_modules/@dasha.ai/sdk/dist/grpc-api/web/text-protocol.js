...
class TextProtocolClient extends base.Client {
    constructor(account, log) {
        super(sdk_grpc_pb.TextProtocolClient, account, log.child({ ...log.defaultMeta, service: "chat" }));
    }
    sendText(request) {
    ...
    messageStream(request) {
        return new base.Stream((stream) => {
            ...
            grpcRequest.setFeatures(new sdk_pb.TextProtocolFeatures().setMultichannel(false));
            ...
            grpcStream.on("data", (grpcMessage) => {
                const { jobId, text, closed, inWait } = grpcMessage.toObject();
                if (text !== undefined) {
